...
(defn init-storage!
  ...
  (let [storage-type (config/storage-type)]
    ...
    (case storage-type
      :s3 (do
            (require 'mdbrain.object-store.s3)
            (let [create-fn (resolve 'mdbrain.object-store.s3/create-s3-store)]
              (set-store! (create-fn))
              (log/info "S3 storage initialized successfully")))
              :local (do
                       (require 'mdbrain.object-store.local)
                       (let [create-fn (resolve 'mdbrain.object-store.local/create-local-store)]
                         (set-store! (create-fn))
