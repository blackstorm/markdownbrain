(ns mdbrain.app-host-binding-test
  (:require
...
(deftest test-app-host-binding-middleware
  (testing "Missing Host returns 400"
    (let [handler (middleware/wrap-app-host-binding (fn [_] {:status 200 :body "ok"}))
  ...
  (testing "Bound Host passes vault context through"
    (let [tenant-id (utils/generate-uuid)
          ...
          handler (middleware/wrap-app-host-binding
                   (fn [request]
                     {:status 200
                      :body {:domain (:mdbrain/domain request)
                             :vault-id (get-in request [:mdbrain/vault :id])}}))
          req (-> (mock/request :get "/")
  ...
  (testing "Reserved paths bypass host binding"
    (let [handler (middleware/wrap-app-host-binding (fn [_] {:status 418 :body "teapot"}))
