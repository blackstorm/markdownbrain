(ns mdbrain.link-parser-test
  "链接解析器测试 - 服务端统一解析 [[link]] 语法
   ...
   4. 生成可直接写入 note_links 表的数据"
  (:require [clojure.test :refer :all]
            [mdbrain.link-parser :as lp]))
...
(deftest test-resolve-links
  (let [;; 模拟 vault 中的笔记
        notes [{:client-id "client-a" :path "Note A.md"}
    ...
    (testing "通过路径解析到 target_client_id"
      (let [links [{:original "[[Note A]]" :path "Note A" :display "Note A" :anchor nil :link-type "link"}]
            resolved (lp/resolve-links links notes)]
        (is (= 1 (count resolved)))
    ...
    (testing "resolved 结果包含写入 note_links 所需的所有字段"
...
;;; ============================================================
;;; 3. 端到端：从 content 到 note_links 数据
;;; ============================================================

(deftest test-parse-and-resolve
  (let [notes [{:client-id "client-hello" :path "Hello world.md"}
