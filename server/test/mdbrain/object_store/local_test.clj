...
(defn- create-temp-dir
  ...
  []
  (let [temp-path (Files/createTempDirectory "mdbrain-test-" (into-array FileAttribute []))]
    (.toFile temp-path)))
...
(defn with-temp-storage
  ...
  (let [temp-dir (create-temp-dir)
    ...
    (try
      ...
      (with-redefs [mdbrain.config/storage-config (constantly {:type :local
      ...
      (try
        (with-redefs [mdbrain.config/storage-config (constantly {:type :local
          ...
          (let [store (local/create-local-store)]
            ...
            (is (instance? mdbrain.object_store.local.LocalObjectStore store))))
        (finally
          (delete-recursively temp-dir))))))
