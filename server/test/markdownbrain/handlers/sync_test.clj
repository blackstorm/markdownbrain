...
(deftest test-sync-file-server-parses-links
  (testing "服务端从 content 解析链接并存入 document_links"
    (let [tenant-id (utils/generate-uuid)
          ...
          ;; 先创建目标文档
          _ (db/upsert-document! (utils/generate-uuid) tenant-id vault-id "Target Note.md" "target-client-id"
      ...
      (let [links (db/get-document-links vault-id "source-client-id")]
        (is (= 1 (count links)))
  ...
  (testing "服务端解析多个链接"
    (let [tenant-id (utils/generate-uuid)
          ...
          ;; 创建目标文档
          _ (db/upsert-document! (utils/generate-uuid) tenant-id vault-id "Note A.md" "client-a"
                                 "# A" "{}" "ha" "2025-01-01T00:00:00Z")
          _ (db/upsert-document! (utils/generate-uuid) tenant-id vault-id "Note B.md" "client-b"
          ...
          response (sync/sync-file request)]
      (is (= 200 (:status response)))
      (let [links (db/get-document-links vault-id "hub-client-id")]
        (is (= 2 (count links)))
  ...
  (testing "链接目标不存在时不存储（broken link 在渲染时处理）"
    (let [tenant-id (utils/generate-uuid)
      ...
      (let [links (db/get-document-links vault-id "orphan-client-id")]
        (is (empty? links)))))
  ...
  (testing "更新文档时重新解析链接"
    (let [tenant-id (utils/generate-uuid)
          ...
          ;; 创建目标文档
          _ (db/upsert-document! (utils/generate-uuid) tenant-id vault-id "Old Target.md" "old-target"
                                 "# Old" "{}" "ho" "2025-01-01T00:00:00Z")
          _ (db/upsert-document! (utils/generate-uuid) tenant-id vault-id "New Target.md" "new-target"
          ...
          _ (sync/sync-file (sync-request-with-header sync-key
                                                      :body {:path "source.md"
                                                            ...
                                                            :action "create"}))
          ;; 验证第一次的链接
          links-v1 (db/get-document-links vault-id "update-source")
          ...
          _ (sync/sync-file (sync-request-with-header sync-key
                                                      :body {:path "source.md"
                                                            ...
                                                            :action "modify"}))
          ;; 验证链接已更新
          links-v2 (db/get-document-links vault-id "update-source")]
      (is (= 1 (count links-v2)))
  ...
  (testing "无链接的文档不创建 document_links 记录"
    (let [tenant-id (utils/generate-uuid)
          ...
          response (sync/sync-file request)]
      (is (= 200 (:status response)))
      (let [links (db/get-document-links vault-id "plain-client-id")]
        (is (empty? links))))))
...
(deftest test-sync-file-rename
  (testing "File rename - 同 hash 不同 path 时更新路径"
    (let [tenant-id (utils/generate-uuid)
          ...
          _ (db/upsert-document! doc-id tenant-id vault-id "old-name.md" "rename-client-id"
      ...
      ;; 验证路径已更新
      (let [doc (db/get-document-by-client-id vault-id "rename-client-id")]
  ...
  (testing "File rename - 旧路径不再存在"
    (let [tenant-id (utils/generate-uuid)
          ...
          doc-id (utils/generate-uuid)
          _ (db/upsert-document! doc-id tenant-id vault-id "before.md" "path-client-id"
      ...
      ;; 旧路径应该找不到文档
      (is (nil? (db/get-document-by-path vault-id "before.md")))
      ;; 新路径应该能找到
      (is (some? (db/get-document-by-path vault-id "after.md")))))
  ...
  (testing "Same hash and path - 跳过更新"
    (let [tenant-id (utils/generate-uuid)
          ...
          doc-id (utils/generate-uuid)
          _ (db/upsert-document! doc-id tenant-id vault-id "unchanged.md" "unchanged-client-id"
